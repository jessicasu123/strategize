...
class PieceAtMaxObjectsMoveTest {
    ...
    GamePiece selfPiece = new GamePiece(1, c1,3, directions, new ArrayList<>(), new ArrayList<>(), new ArrayList<>());
    ...
    void completeMoveType() {
        ...
        pieceAtMaxObjectsMove.completeMoveType(selfPiece, c2, neighbors, 1, 1);
        assertEquals(0, selfPiece2.getState());
        assertEquals(0, selfPiece2.getNumObjects());
        assertEquals(1, selfPiece.getState());
        assertEquals(3, selfPiece.getNumObjects());

        // else: it is not modified
        pieceAtMaxObjectsMove.completeMoveType(selfPiece, c3, neighbors, 1, 1);
